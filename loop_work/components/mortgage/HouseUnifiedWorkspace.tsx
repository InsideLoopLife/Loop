...
type Tab = "overview" | "property" | "rates" | "affordability" | "moving";
type ModalState =
  | null
  | { type: "add_home" }
...
export function HouseUnifiedWorkspace(props: Props) {
  ...
  const bankRate = Number(
    props.boeBenchmarks.find((row) => row.term_type === "bank_rate")?.rate_percent || 0,
  );
        ...
        <div className="min-w-0">
          {tab === "overview" ? (
            <div className="mx-auto w-full max-w-[1540px] space-y-6 lg:space-y-7">
              <header><p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-600">House overview</p><h1 className="mt-1 text-3xl font-bold tracking-tight sm:text-4xl">Your home, in one place</h1><p className="mt-2 text-sm text-slate-500">Property, mortgage, affordability and market context in one live view.</p></header>

              <section className="grid grid-cols-2 gap-2 sm:gap-3 xl:grid-cols-4">
                {[["Current mortgage",formatMoney(mortgage.balance),`${ltv.toFixed(1)}% LTV`],["Monthly payment",formatMoney(mortgage.payment),deal?`${Number(deal.interest_rate).toFixed(2)}% · ${deal.lender || ""}`:"No mortgage"],["Home value",formatMoney(value.mid),`${formatMoney(value.low)} – ${formatMoney(value.high)}`],["Affordability",`${affordability.score}/100`,affordability.label]].map(([label,main,helper]) => <article key={label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:min-h-[122px] lg:p-5"><p className="text-[10px] font-bold uppercase text-slate-400">{label}</p><p className="mt-2 text-2xl font-bold">{main}</p><p className="mt-1 text-[11px] font-semibold text-slate-500">{helper}</p></article>)}
              </section>

              <section className="grid gap-3 md:grid-cols-2">
                <article className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-5 shadow-sm"><p className="text-[10px] font-bold uppercase text-emerald-700">Bank of England</p><p className="mt-2 text-3xl font-bold">{bankRate ? `${bankRate.toFixed(2)}%` : "Pending"}</p><p className="mt-1 text-xs text-slate-600">Current Bank Rate · market context.</p></article>
                <article className="rounded-2xl border border-violet-200 bg-violet-50/70 p-5 shadow-sm"><p className="text-[10px] font-bold uppercase text-violet-700">BoE mortgage benchmark</p><p className="mt-2 text-3xl font-bold">{benchmark ? `${Number(benchmark.rate_percent).toFixed(2)}%` : "Pending"}</p><p className="mt-1 text-xs text-slate-600">Nearest 2-year fixed benchmark for your LTV.</p></article>
              </section>
              ...
              <section className="overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white shadow-sm">
                <div className="grid lg:grid-cols-[1.15fr_.85fr]">
                  <div className="relative min-h-[280px] overflow-hidden bg-slate-100 sm:min-h-[360px] lg:min-h-[470px]">
                    ...
                    <div className={`absolute right-4 top-4 rounded-2xl p-4 shadow-lg ring-1 ${affordability.tone}`}><p className="text-[10px] font-bold uppercase">Affordability</p><p className="text-3xl font-bold">{affordability.score}/100</p><p className="text-[11px] font-semibold">{affordability.label}</p></div>
