    ...
    <form action={action} className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <label className="block">
          ...
          <select name="person_id" defaultValue={lockedPersonId ?? initialValues?.person_id ?? ""} disabled={Boolean(lockedPersonId)} required={requirePerson} className={inputClass}>
      ...
      {pensionMethod !== "none" ? (
        <div className="rounded-3xl border border-blue-100 bg-blue-50/60 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-blue-700 ring-1 ring-blue-100">Used by projections</span>
          </div>
          ...
          <label className="mt-4 flex items-start gap-3 rounded-2xl bg-white p-4 ring-1 ring-blue-100">
            ...
            <span><span className="block text-sm font-black text-slate-950">Employer adds its NI saving to the pension</span><span className="mt-1 block text-xs font-medium text-slate-500">Only applies to salary sacrifice. The rate and share remain editable because employer schemes differ.</span></span>
          </label>
        </div>
      ) : null}
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {maternityMonths.map((month) => (
                  <div key={month.month} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <p className="mt-1 text-[11px] leading-4 text-slate-500">Gross {formatMoneyExact(month.grossAmount)}. {month.explanation}</p>
                  </div>
                ))}
              </div>
            </div>
