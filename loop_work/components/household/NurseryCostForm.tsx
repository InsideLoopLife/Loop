  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <label className="block">
        ...
        <select name="child_id" defaultValue={lockedChildId ?? initialValues?.child_id ?? ""} disabled={Boolean(lockedChildId)} className={inputClass}>
      ...
      <label className="block">
        ...
        <select name="bill_person_id" defaultValue={initialValues?.bill_person_id ?? ""} className={inputClass}>
          {hasHousehold ? <option value="">🏠 Household / shared</option> : null}
          {billPersonOptions.map((person) => (
            <option key={person.id} value={person.id}>{person.name}</option>
          ))}
        </select>
        <p className="mt-1 text-xs text-slate-500">Choose Household/shared for nursery fees or family costs that should not sit against one person.</p>
      </label>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <label className="block">
          <span className="text-sm font-medium text-slate-700">Payment timing</span>
          <select name="payment_timing" defaultValue={initialValues?.payment_timing ?? "fixed_day"} className={inputClass}>
            <option value="fixed_day">Fixed day of month</option>
            <option value="last_workday">Last working day of month</option>
          </select>
        </label>
        <label className="block">
          <span className="text-sm font-medium text-slate-700">Payment day</span>
          <input name="payment_day_of_month" defaultValue={initialValues?.payment_day_of_month ?? 1} className={inputClass} type="number" min="1" max="31" />
        </label>
        <label className="block">
          <span className="text-sm font-medium text-slate-700">If weekend / bank holiday</span>
          <select name="payment_adjustment" defaultValue={initialValues?.payment_adjustment ?? "previous_workday"} className={inputClass}>
