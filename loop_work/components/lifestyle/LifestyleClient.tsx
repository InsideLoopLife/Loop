...
import { Baby, CalendarClock, CheckCircle2, ClipboardList, Copy, Download, HeartPulse, Plus, ReceiptText, RotateCcw, Salad, Share2, ShieldCheck, ShoppingBasket, Sparkles, Umbrella, Utensils } from "lucide-react";
import { FormInput } from "@/components/FormInput";
import { BillWizard } from "@/components/lifestyle/BillWizard";
import { MealWizard } from "@/components/lifestyle/MealWizard";
...
type LifestyleFoodLog = { eaten_on: string; calories: number | null; protein_g: number | null; fibre_g: number | null; salt_g: number | null; caffeine_mg: number | null; drink_volume_ml: number | null; meal_slot: string | null; label: string | null; };
...
type Props = { people: Person[]; bills: DealBill[]; supermarkets: Supermarket[]; meals: Meal[]; foodLogs?: LifestyleFoodLog[] };
...
function parseIngredientAmount(line: string) {
  ...
  const match = cleaned.match(/^((?:\d+(?:\.\d+)?|\d+\/\d+|a|an|one|two|three|four|five|six|pinch|handful|shot|shots|tbsp|tsp)\s*(?:x|g|kg|ml|l|tbsp|tsp|teaspoons?|tablespoons?|shots?|cloves?|large|small|medium|pinch|handful|slices?)?\s*)\b(.+)$/i);
  if (!match) return { quantity: "", name: cleaned };
  return { quantity: match[1].trim(), name: match[2].trim() || cleaned };
}

function normaliseIngredientName(name: string) {
  return name
    .toLowerCase()
    .replace(/\b(fresh|finely|roughly|chopped|sliced|grated|large|small|medium|unsweetened|organic|optional|for garnish|to taste)\b/g, " ")
    .replace(/[^a-z0-9% ]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

const roughIngredientPrices: Array<{ pattern: RegExp; price: number; confidence: string }> = [
  { pattern: /pancetta|bacon|guanciale|prosciutto/, price: 2.50, confidence: "typical 100-150g pack" },
  { pattern: /spaghetti|pasta|linguine|penne/, price: 1.10, confidence: "typical 500g dry pasta" },
  { pattern: /egg/, price: 1.20, confidence: "share of a 6-pack" },
  { pattern: /parmesan|pecorino|grana padano/, price: 2.20, confidence: "typical cheese portion" },
  { pattern: /milk|gold top|whole milk|semi skimmed|almond milk|oat milk/, price: 1.65, confidence: "typical 1L milk/carton" },
  { pattern: /espresso|coffee/, price: 0.45, confidence: "rough per-home serving" },
  { pattern: /garlic|onion|lemon|lime/, price: 0.35, confidence: "single fresh item" },
  { pattern: /chicken|beef|mince|steak|salmon|cod|pork/, price: 4.50, confidence: "rough protein pack" },
  { pattern: /tomato|passata|sauce/, price: 1.00, confidence: "typical tin/jar" },
  { pattern: /salt|pepper|spice|cinnamon|vanilla|butter|oil/, price: 0.20, confidence: "pantry-use estimate" },
];

function estimateIngredientCost(name: string) {
  const hit = roughIngredientPrices.find((item) => item.pattern.test(name.toLowerCase()));
  return hit || { price: 0.75, confidence: "indicative fallback" };
}

function shoppingItemsFromMeals(meals: Meal[], supermarkets: Supermarket[], freehandLines: string[] = []) {
  const seen = new Map<string, ShoppingItem>();

  function addLine(rawLine: string, mealLabel: string, supermarket: string, sourceId: string, index: number) {
    const clean = cleanIngredientLine(rawLine);
    if (!clean) return;
    const parsed = parseIngredientAmount(clean);
    const normalised = normaliseIngredientName(parsed.name || clean);
    const key = normalised || clean.toLowerCase();
    const cost = estimateIngredientCost(key);
    const existing = seen.get(key);
    const nextLabel = parsed.name || clean;
    const nextQuantity = parsed.quantity || "";
    if (existing) {
      seen.set(key, {
        ...existing,
        quantityText: [existing.quantityText, nextQuantity].filter(Boolean).join(" + "),
        mealLabel: Array.from(new Set([...existing.mealLabel.split(", "), mealLabel])).filter(Boolean).join(", "),
        estimatedCost: Number((Number(existing.estimatedCost || 0) + cost.price).toFixed(2)),
        costConfidence: existing.costConfidence === cost.confidence ? existing.costConfidence : "combined indicative pricing",
      });
      return;
    }
    seen.set(key, {
      id: `${sourceId}-${index}-${key.replace(/[^a-z0-9]+/g, "-")}`,
      label: nextLabel,
      normalisedLabel: key,
      quantityText: nextQuantity,
      mealLabel,
      supermarket,
      estimatedCost: cost.price,
      costConfidence: cost.confidence,
    });
  }

  meals.forEach((meal) => {
    const lines = String(meal.ingredients || "").split(/\r?\n|,/).map(cleanIngredientLine).filter(Boolean);
    lines.forEach((line, index) => addLine(line, meal.label, supermarketName(supermarkets, meal.supermarket_id), meal.id, index));
  });
  freehandLines.forEach((line, index) => addLine(line, "Added manually", "Indicative", "manual", index));
...
function buildAppleNotesShoppingText(items: ShoppingItem[], meals: Meal[]) {
  ...
  const lines = [
    ...
    `Meals: ${mealNames}`,
    "",
    ...items.map((item) => `☐ ${[item.quantityText, item.label].filter(Boolean).join(" ")}${item.estimatedCost ? ` — ~£${item.estimatedCost.toFixed(2)}` : ""}`),
  ];
...
function ShoppingListBuilder({ meals, supermarkets }: { meals: Meal[]; supermarkets: Supermarket[] }) {
  ...
  const [freehandIngredient, setFreehandIngredient] = useState("");
  const [freehandLines, setFreehandLines] = useState<string[]>([]);
  ...
  const items = useMemo(
    () => shoppingItemsFromMeals(selectedMeals, supermarkets, freehandLines),
    [selectedMeals, supermarkets, freehandLines]
  );
  const roughTotal = useMemo(() => items.reduce((sum, item) => sum + Number(item.estimatedCost || 0), 0), [items]);
  ...
  function addFreehandIngredient() {
    const clean = cleanIngredientLine(freehandIngredient);
    if (!clean) return;
    setFreehandLines((current) => [...current, clean]);
    setFreehandIngredient("");
  }
    ...
    <div className="rounded-[2rem] border border-white/70 bg-white/90 p-5 shadow-lg">
      ...
      <div className="mt-4 rounded-3xl border border-orange-100 bg-orange-50/70 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
          <label className="block flex-1"><span className="text-sm font-black text-orange-950">Add ingredients freehand</span><input value={freehandIngredient} onChange={(event) => setFreehandIngredient(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addFreehandIngredient(); } }} placeholder="2 shots espresso, Graham's Gold Top milk, 100g pancetta..." className="mt-1 w-full rounded-2xl border border-orange-100 bg-white px-4 py-3 text-sm font-semibold outline-none ring-orange-500 focus:ring-2" /></label>
          <button type="button" onClick={addFreehandIngredient} className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Add to list</button>
        </div>
        <p className="mt-2 text-xs font-semibold text-orange-900">Ingredients are combined where names match and given a rough indicative cost. Exact supermarket matching can be layered in later with a saved supermarket preference/API.</p>
        {freehandLines.length ? <div className="mt-3 flex flex-wrap gap-2">{freehandLines.map((line, idx) => <button key={`${line}-${idx}`} type="button" onClick={() => setFreehandLines((current) => current.filter((_, i) => i !== idx))} className="rounded-full bg-white px-3 py-1 text-xs font-black text-orange-700 ring-1 ring-orange-100">{line} ×</button>)}</div> : null}
      </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs font-semibold text-slate-500">{checkedItemIds.length} of {items.length} ticked · rough total £{roughTotal.toFixed(2)}</p>
            </div>
            ...
            {items.map((item) => {
              ...
              return (
                <button key={item.id} type="button" onClick={() => toggleItem(item.id)} className={`flex w-full items-start gap-3 rounded-2xl border p-3 text-left transition ${checked ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-slate-200 bg-slate-50 text-slate-800 hover:bg-slate-100"}`}>
                  ...
                  <span className="min-w-0 flex-1"><span className={`block text-sm font-black ${checked ? "line-through" : ""}`}>{[item.quantityText, item.label].filter(Boolean).join(" ")}</span><span className="text-xs font-semibold opacity-70">{item.mealLabel}</span>{item.estimatedCost ? <span className="mt-1 block text-xs font-black text-orange-700">~£{item.estimatedCost.toFixed(2)} · {item.costConfidence}</span> : null}</span>
                </button>
              );
            })}
...
function LifestyleOSToday({ foodLogs }: { foodLogs: LifestyleFoodLog[] }) {
  const todayKey = today();
  const todayLogs = foodLogs.filter((log) => log.eaten_on === todayKey);
  const weekLogs = foodLogs.filter((log) => {
    const days = Math.round((new Date(`${todayKey}T00:00:00`).getTime() - new Date(`${log.eaten_on}T00:00:00`).getTime()) / 86400000);
    return days >= 0 && days < 7;
  });
  const monthLogs = foodLogs;
  const totals = todayLogs.reduce((acc, log) => ({
    calories: acc.calories + n(log.calories), protein: acc.protein + n(log.protein_g), fibre: acc.fibre + n(log.fibre_g), salt: acc.salt + n(log.salt_g), caffeine: acc.caffeine + n(log.caffeine_mg), fluids: acc.fluids + n(log.drink_volume_ml),
  }), { calories: 0, protein: 0, fibre: 0, salt: 0, caffeine: 0, fluids: 0 });
  const checks = [
    { label: "Logged today", pass: todayLogs.length > 0, detail: `${todayLogs.length} entry(s)` },
    { label: "Hydration context", pass: totals.fluids >= 500, detail: `${Math.round(totals.fluids)}ml logged` },
    { label: "Protein signal", pass: totals.protein >= 60, detail: `${Math.round(totals.protein)}g protein` },
    { label: "Fibre signal", pass: totals.fibre >= 18, detail: `${totals.fibre.toFixed(1)}g fibre` },
  ];
  const watchouts = [
    totals.caffeine > 300 ? `Caffeine is already ${Math.round(totals.caffeine)}mg — watch timing later in the day.` : null,
    totals.salt > 5 ? `Salt is near the UK daily guide at ${totals.salt.toFixed(1)}g.` : null,
    totals.fibre < 10 && todayLogs.length ? "Fibre is light so far — fruit, beans, oats or wholegrains would help." : null,
  ].filter(Boolean);
  return <section className="rounded-[2.5rem] border border-emerald-100 bg-emerald-50/80 p-5 shadow-xl">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-emerald-700">Lifestyle OS today</p><h2 className="mt-1 text-3xl font-black text-slate-950">Daily loop, not macro overload</h2><p className="mt-1 max-w-3xl text-sm font-semibold text-slate-600">Nutrition detail stays in the Nutrition page. This view is just green ticks, encouragement and trend direction.</p></div><div className="rounded-full bg-white px-4 py-2 text-sm font-black text-emerald-800">Week {weekLogs.length} logs · Month {monthLogs.length} logs</div></div>
    <div className="mt-5 grid gap-3 md:grid-cols-4">{checks.map((item) => <div key={item.label} className="rounded-[1.5rem] bg-white p-4 shadow-sm"><div className={`mb-3 grid h-10 w-10 place-items-center rounded-full ${item.pass ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}><CheckCircle2 className="h-5 w-5" /></div><p className="font-black text-slate-950">{item.label}</p><p className="mt-1 text-xs font-bold text-slate-500">{item.detail}</p></div>)}</div>
    <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr]"><div className="rounded-[1.75rem] bg-white p-4"><p className="text-sm font-black text-slate-950">Today’s encouragement</p><p className="mt-2 text-sm font-semibold text-slate-600">{todayLogs.length ? "You have started the loop today. Keep the next choice simple and useful rather than perfect." : "Log the first drink or meal and LOOP can start spotting today’s pattern."}</p>{watchouts.length ? <ul className="mt-3 list-disc space-y-1 pl-5 text-sm font-semibold text-orange-800">{watchouts.map((item) => <li key={String(item)}>{item}</li>)}</ul> : <p className="mt-3 text-sm font-semibold text-emerald-700">Nothing major to flag from today’s summary yet.</p>}</div><div className="rounded-[1.75rem] bg-white p-4"><p className="text-sm font-black text-slate-950">Want to know more?</p><p className="mt-1 text-xs font-semibold text-slate-500">This will use the AI allowance for the user’s tier.</p><div className="mt-3 flex gap-2"><input disabled placeholder="Ask: why is my fibre trend low this week?" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold" /><button disabled className="rounded-2xl bg-slate-200 px-4 py-3 text-sm font-black text-slate-500">Ask LOOP</button></div><p className="mt-2 text-xs font-semibold text-slate-400">AI insight wiring can be switched on after tier/token checks are connected.</p></div></div>
  </section>;
}

export function LifestyleClient({ people, bills, supermarkets, meals, foodLogs = [] }: Props) {
  const [tab, setTab] = useState<Tab>("deals");
  const [modal, setModal] = useState<Modal>(null);
  const [openMenu, setOpenMenu] = useState(false);

  const children = useMemo(() => people.filter((person) => String(person.relationship || "").toLowerCase() === "child"), [people]);
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-7 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <LifestyleOSToday foodLogs={foodLogs} />

      {children.length > 0 ? (
        <section className="overflow-hidden rounded-[2.25rem] border border-white/70 bg-slate-950 text-white shadow-2xl shadow-slate-950/15">
          <div className="grid gap-5 p-6 lg:grid-cols-[1fr_auto] lg:items-center">
            <div className="flex items-start gap-4">
              <div className="rounded-3xl bg-emerald-300/15 p-3 text-emerald-200"><Baby className="h-6 w-6" /></div>
              <div>
                <p className="text-xs font-black uppercase tracking-[0.24em] text-emerald-300">Family planning calendar</p>
                <h2 className="mt-1 text-3xl font-black tracking-tight">School holidays, nursery closures and annual leave cover.</h2>
                <p className="mt-2 max-w-3xl text-sm font-semibold text-slate-300">Detected {children.length} child profile{children.length === 1 ? "" : "s"}. Plan who covers each school/nursery holiday, track leave allowance and spot gaps before they become chaos.</p>
                <div className="mt-4 flex flex-wrap gap-2 text-xs font-black text-slate-200">
                  <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-2"><Umbrella className="h-3.5 w-3.5" /> Holiday cover</span>
                  <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-2"><ShieldCheck className="h-3.5 w-3.5" /> Leave split</span>
                  <span className="rounded-full bg-orange-400/15 px-3 py-2 text-orange-200">No AI/web-search by default</span>
                </div>
              </div>
            </div>
            <Link href="/lifestyle/family-planning" className="inline-flex items-center justify-center rounded-full bg-white px-5 py-3 text-sm font-black text-slate-950 shadow-xl shadow-black/20 transition hover:-translate-y-0.5">Open family planner</Link>
          </div>
        </section>
      ) : null}
      ...
      <div className="flex flex-wrap gap-2 rounded-full border border-slate-200 bg-white/80 p-1 shadow-inner">
        {[
          ...
          { id: "health", label: "Lifestyle summary", icon: HeartPulse },
        ].map((item) => {
      ...
      {modal?.type === "bill" ? <ModalShell title="Add bill / renewal" description="Track contracts so the overview can remind you when to compare better deals." onClose={() => setModal(null)}><BillWizard people={people} /></ModalShell> : null}
      {modal?.type === "edit-bill" ? <ModalShell title={`Edit ${modal.bill.provider}`} description="Update costs, renewal date or comparison details." onClose={() => setModal(null)}><BillWizard people={people} bill={modal.bill} /></ModalShell> : null}
      ...
      {modal?.type === "meal" ? <ModalShell title="Add meal / recipe" description="Manual first: paste a recipe URL, image reference, ingredients and rough macros." onClose={() => setModal(null)}><MealWizard people={people} supermarkets={supermarkets} /></ModalShell> : null}
      {modal?.type === "edit-meal" ? <ModalShell title={`Edit ${modal.meal.label}`} onClose={() => setModal(null)}><MealWizard people={people} supermarkets={supermarkets} meal={modal.meal} /></ModalShell> : null}
    </main>
  );
}
