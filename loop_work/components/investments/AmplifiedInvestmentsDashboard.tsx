...
function DiversificationBars({ holdings, snapshots, totalValue, period, dark, onOpenOther, onOpenHolding }: { holdings: InvestmentHolding[]; snapshots: InvestmentSnapshot[]; totalValue: number; period: InvestmentPeriod; dark: boolean; onOpenOther: (items: InvestmentHolding[]) => void; onOpenHolding: (holding: InvestmentHolding) => void; }) {
  ...
  const visible = allItems.length > 10 ? allItems.slice(0, 9) : allItems;
  const remainder = allItems.length > 10 ? allItems.slice(9) : [];
        ...
        {!hideHeader ? (
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button type="button" onClick={() => setTheme((val) => (val === "dark" ? "light" : "dark"))} className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-semibold shadow-sm transition ${dark ? "border-white/10 bg-white/5 text-white hover:bg-white/10" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}>
