...
import { dedupeHouseholdPeople, getActiveHouseholdContext, householdPeopleOrFilter, visibleDataOrFilter } from "@/lib/auth/household-context";
...
type Person = { id: string; name: string; relationship: string; user_id?: string | null; linked_user_id?: string | null; email?: string | null; invite_email?: string | null; birth_date?: string | null; account_status?: string | null; active_until?: string | null };
...
type FinancialAccount = { id: string; owner_person_id?: string | null; name: string; account_type: string; current_balance: number; is_liability: boolean; provider?: string | null };
...
export default async function NetWorthPage() {
  ...
  const [{ data: manualAssets }, { data: manualLiabilities }, { data: people }, { data: homes }, { data: owners }, { data: valuations }, { data: deals }, { data: pensionAccounts }, { data: pensionFunds }, { data: investmentAccounts }, { data: investmentHoldings }, { data: financialAccounts }] = await Promise.all([
    supabase.from("assets").select("id, person_id, name, value, type, source_type").or(householdVisibleFilter).order("created_at", { ascending: false }).returns<Asset[]>(),
    supabase.from("liabilities").select("id, person_id, name, balance, type, source_type").or(householdVisibleFilter).order("created_at", { ascending: false }).returns<Liability[]>(),
    supabase.from("people").select("id, name, relationship, user_id, linked_user_id, email, invite_email, birth_date, account_status, active_until").or(householdPeopleOrFilter(householdContext)).order("relationship").order("name").returns<Person[]>(),
    supabase.from("homes").select("id, label, property_value, estimated_value_low, estimated_value_mid, estimated_value_high").or(householdVisibleFilter).returns<Home[]>(),
    supabase.from("home_owners").select("home_id, person_id").or(householdVisibleFilter).returns<Owner[]>(),
    supabase.from("home_valuation_sources").select("home_id, valuation_mid, valuation_amount, confidence").or(householdVisibleFilter).returns<Valuation[]>(),
    supabase.from("home_mortgage_deals").select("id, home_id, lender, balance, balance_as_of_date, interest_rate, term_years, monthly_payment_override, repayment_type, start_date").or(householdVisibleFilter).returns<Deal[]>(),
    supabase.from("pension_accounts").select("id, person_id, label, provider, current_value").or(householdVisibleFilter).returns<PensionAccount[]>(),
    supabase.from("pension_funds").select("pension_account_id, current_value, units, unit_price").or(householdVisibleFilter).returns<PensionFund[]>(),
    supabase.from("investment_accounts").select("id, person_id, label, provider, account_type").or(householdVisibleFilter).returns<InvestmentAccount[]>(),
    supabase.from("investment_holdings").select("investment_account_id, asset_name, units, latest_price").or(householdVisibleFilter).returns<InvestmentHolding[]>(),
    supabase.from("financial_accounts").select("id, name, owner_person_id, account_type, current_balance, is_liability, provider").or(householdVisibleFilter).returns<FinancialAccount[]>(),
  ]);
  ...
  const accountAssets: Asset[] = (financialAccounts ?? []).filter((account) => !account.is_liability).map((account) => ({
    id: `financial-account-${account.id}`,
    person_id: account.owner_person_id || null,
    name: `${account.provider ? `${account.provider} · ` : ""}${account.name}`,
    value: Math.abs(Number(account.current_balance || 0)),
    type: account.account_type === "savings" ? "cash" : account.account_type,
    source_type: "financial_account",
  }));

  const accountLiabilities: Liability[] = (financialAccounts ?? []).filter((account) => account.is_liability).map((account) => ({
    id: `financial-account-liability-${account.id}`,
    person_id: account.owner_person_id || null,
    name: `${account.provider ? `${account.provider} · ` : ""}${account.name}`,
  ...
  const allAssets = [...autoAssets, ...pensionAssets, ...investmentAssets, ...accountAssets, ...(manualAssets ?? [])];
  const allLiabilities = [...autoLiabilities, ...accountLiabilities, ...(manualLiabilities ?? [])];

  return (
    <>
      <Nav />
      {allAssets.length + allLiabilities.length === 0 ? (
        <main className="mx-auto w-[95vw] max-w-[2000px] px-4 py-6 sm:px-6 lg:px-8">
          <PageLandingExperience kind="net-worth" />
        </main>
      ) : null}
      <NetWorthClient currentUserId={user.id} householdName={householdContext.householdName} people={dedupeHouseholdPeople(people ?? [], user.id)} assets={allAssets} liabilities={allLiabilities} />
    </>
  );
}
