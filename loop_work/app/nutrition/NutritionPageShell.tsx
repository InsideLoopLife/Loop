...
export type NutritionPageTab = "overview" | "recipes" | "food-log" | "meal-cards";

export async function NutritionPageShell({ searchParams, tab = "overview" }: { searchParams?: Promise<{ date?: string; open?: string; meal?: string }>; tab?: NutritionPageTab }) {
  ...
  if (linkedUserIds.length > 0) {
    const profileClient = hasSupabaseAdminKey() ? createAdminClient() : supabase;
    const { data: linkedProfiles } = await profileClient
