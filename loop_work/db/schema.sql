...
alter table affordability_scenarios add column if not exists request_text text;
alter table affordability_scenarios add column if not exists scenario_kind text;
alter table affordability_scenarios add column if not exists assistant_summary text;
alter table affordability_scenarios add column if not exists questions_json jsonb not null default '[]'::jsonb;
alter table affordability_scenarios add column if not exists assumptions_json jsonb not null default '[]'::jsonb;
alter table affordability_scenarios add column if not exists answer_log jsonb not null default '{}'::jsonb;
...
create table if not exists investment_holdings (
  ...
  exchange text,
  group_label text,
  asset_kind text not null default 'share' check (asset_kind in ('share', 'etf', 'fund', 'crypto', 'other')),
  isin text,
  price_polling_enabled boolean not null default true,
...
create index if not exists investment_price_snapshots_user_holding_at_idx on investment_price_snapshots(user_id, holding_id, snapshot_at desc);
...
-- V27.6: Financial Flow lifecycle / renewal tracking and student loan accounts.
alter table planned_items
