...
export function buildRuntimeReadiness(env: NodeJS.ProcessEnv): ReadinessItem[] {
  ...
  const hasCreatorEmails = Boolean(env.APP_CREATOR_EMAILS);
  ...
  return [
    ...
    {
      key: "creator-admin",
      title: "Creator/admin access declared",
      status: hasCreatorEmails ? "pass" : "warn",
      detail: hasCreatorEmails
        ? "APP_CREATOR_EMAILS is set for creator-only admin access."
        : "No creator email allow-list set. Admin access should be explicitly configured.",
      action: "Set APP_CREATOR_EMAILS=your@email.com or insert your email into app_admin_users.",
    },
    {
      key: "email-provider",
