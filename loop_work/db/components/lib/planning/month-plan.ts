...
export type MonthPlanLine = { label: string; value: number; href: string; helper: string };
...
function getChildCostMonthlyAmount(cost: ChildCostForPlan, month: string) {
...
export function buildMonthPlan({
  ...
  if (savingsTarget > 0) {
    outgoingItems.push({ label: "Savings target", value: savingsTarget, href: "/dashboard", helper: "Monthly saving plan" });
  }
