...
async function saveProviderPositionSnapshot(
  ...
  const price = Number(args.latestPrice || (units ? value / units : value));
  if (!args.holdingId || !Number.isFinite(value) || value <= 0) return;
  ...
  await supabase
    ...
    .insert({
      ...
      price: Number.isFinite(price) && price > 0 ? price : value,
  ...
  for (const account of selected) {
    ...
    for (const position of positionsToImport) {
      ...
      const holdingPayload = {
        ...
        group_label: position.groupLabel || (position.raw?.synthetic ? "Account value placeholder" : null),
