...
import { getActiveHouseholdContext, householdWriteFields, visibleDataOrFilter } from "@/lib/auth/household-context";
...
async function syncSelfPersonWithAccount(
  ...
  const matchFilter = `linked_user_id.eq.${user.id},relationship.eq.self${email ? `,email.ilike.${email},invite_email.ilike.${email}` : ""}`;
  const existingSelfQuery = supabase
    .from("people")
    .select("id, avatar_url, email, name")
    .or(matchFilter)
    .order("relationship", { ascending: false })
    .limit(1);
  const { data: existingSelf } = await (householdContext.householdId
    ? existingSelfQuery.eq("household_id", householdContext.householdId)
    : existingSelfQuery.eq("user_id", user.id)
  ).maybeSingle();
...
export async function assignChildGuardians(formData: FormData) {
  ...
  await supabase.from("person_guardians").delete().eq("child_person_id", childId).or(visibleDataOrFilter(householdContext));
