...
export default async function InvestmentsPage() {
  ...
  const [peopleResult, pensionAccountsResult, pensionFundsResult, investmentAccountsResult, investmentAccountOwnersResult, investmentPieSettingsResult, investmentHoldingsResult, investmentLotsResult, dbPensionSchemesResult, dbPensionEventsResult, payEventsResult, profileResult, snapTradeConnectionResult] = await Promise.all([
    ...
    supabase.from("pension_accounts").select("id, person_id, label, provider, pension_type, contribution_method, employee_contribution_percent, employer_contribution_percent, employer_ni_topup_percent, employer_ni_topup_enabled, fixed_monthly_contribution, annual_platform_fee_percent, fixed_monthly_fee, current_value, value_as_of_date, source_url, notes").eq("user_id", dataOwnerUserId).order("created_at", { ascending: false }).returns<PensionAccount[]>(),
    ...
    supabase.from("investment_holdings").select("id, investment_account_id, asset_name, ticker, exchange, group_label, asset_kind, isin, units, average_buy_price, latest_price, latest_price_date, currency, annual_asset_fee_percent, target_allocation_percent, price_polling_enabled, price_quote_unit, source_url, native_latest_price, native_currency, native_exchange, latest_fx_rate_to_gbp, latest_fx_source, previous_close_price_gbp, previous_close_native_price, previous_close_native_currency, previous_close_at, day_change_gbp, day_change_percent, day_change_native, day_change_native_percent, cost_basis_status, imported_invested_value, imported_current_value, imported_result_value, imported_account_currency, import_source_type, notes").eq("user_id", dataOwnerUserId).neq("record_status", "archived").order("created_at", { ascending: true }).returns<InvestmentHolding[]>(),
    supabase.from("investment_purchase_lots").select("id, holding_id, purchase_date, units, purchase_price, price_quote_unit, notes").eq("user_id", dataOwnerUserId).order("purchase_date", { ascending: true }).returns<InvestmentLot[]>(),
    supabase.from("defined_benefit_pensions").select("id, person_id, scheme_name, provider, scheme_section, accrual_rate, revaluation_rate_percent, notes").eq("user_id", dataOwnerUserId).order("created_at", { ascending: false }).returns<DbPensionScheme[]>(),
