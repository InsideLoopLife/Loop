...
using (
  ...
  or coalesce(invited_email_hash, '') = encode(digest(lower(coalesce(auth.jwt() ->> 'email', '')), 'sha256'), 'hex')
);
...
begin
  ...
  where (p_invite_id is not null and i.id = p_invite_id)
     ...
     or (p_token is not null and length(trim(p_token)) > 16 and i.token_hash = encode(digest(trim(p_token), 'sha256'), 'hex'))
...
begin
  ...
  where status = 'pending'
    and (
      (p_invite_id is not null and id = p_invite_id)
      or (p_token is not null and token_hash = encode(digest(trim(p_token), 'sha256'), 'hex'))
    )
  order by created_at desc
  limit 1
  for update;
