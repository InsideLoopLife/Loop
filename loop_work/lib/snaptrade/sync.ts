...
function exchangeCode(value: any) {
  return firstText(
    value?.mic_code,
    value?.code,
    value?.exchange?.mic_code,
    value?.exchange?.code,
    value,
  ).toUpperCase();
}

function logoUrlFromPosition(position: any) {
  const instrument = universalInstrumentFromPosition(position);
  const value = firstText(
    position?.logo_url,
    position?.logoUrl,
    position?.symbol?.logo_url,
    position?.symbol?.logoUrl,
    position?.security?.logo_url,
    position?.instrument?.logo_url,
    position?.instrument?.logoUrl,
    position?.universal_symbol?.logo_url,
    position?.universal_symbol?.logoUrl,
    position?.symbol?.universal_symbol?.logo_url,
    position?.symbol?.universal_symbol?.logoUrl,
    position?.symbol?.figi_instrument?.logo_url,
    position?.symbol?.figi_instrument?.logoUrl,
    position?.instrument?.figi_instrument?.logo_url,
    position?.instrument?.figi_instrument?.logoUrl,
    instrument?.figi_instrument?.logo_url,
    instrument?.figi_instrument?.logoUrl,
    instrument?.logo_url,
    instrument?.logoUrl,
  );
...
function stableInstitutionAccountKey(account: {
  ...
  const provider = normaliseProviderKey(
    account.providerName || account.provider || providerNameForAccount(raw),
  );
  const wrapper = normaliseWrapperType(
    account.accountType ||
      account.rawType ||
      raw?.raw_type ||
      raw?.type ||
...
function stableAccountLooseKey(account: {
  ...
  const name = normaliseProviderKey(
    account.name || account.label || account.rawType || "account",
  );
...
function stableAccountDisplayKey(account: {
  ...
  const wrapper = normaliseWrapperType(account.accountType || account.rawType);
  const name = normaliseProviderKey(
    account.name || account.rawType || "account",
  );
...
function dedupeSnapTradePreviews(previews: SnapTradeAccountPreview[]) {
  ...
  for (const preview of previews) {
    ...
    const key =
      institutionKey ||
      stableAccountDisplayKey(preview) ||
      stableAccountLooseKey(preview);
    ...
    winner.alreadyImported =
      existing.alreadyImported || preview.alreadyImported;
...
async function saveProviderPositionSnapshot(
  ...
  const nativePrice = Number(
    args.latestPrice || (units ? nativeValue / units : nativeValue),
  );
  const nativeCurrency =
    String(args.currency || "GBP")
      .trim()
      .toUpperCase() || "GBP";
  if (!args.holdingId || !Number.isFinite(nativeValue) || nativeValue <= 0)
    return;
  const snapshotAt = args.snapshotAt || new Date();
  const converted = await quotePriceToGbp(
    Number.isFinite(nativePrice) && nativePrice > 0 ? nativePrice : nativeValue,
    nativeCurrency,
  ).catch(() => ({
    gbpPrice: nativePrice || nativeValue,
    fxRate: 1,
    fxSource: "provider native fallback",
  }));
  const fxRate = Number((converted as any).fxRate || 1);
  const gbpPrice = Number(
    (converted as any).gbpPrice || nativePrice || nativeValue,
  );
  await supabase
    ...
    .insert({
      ...
      value:
        Number.isFinite(units) && units > 0
          ...
          : nativeValue * fxRate,
      native_price:
        Number.isFinite(nativePrice) && nativePrice > 0
          ? nativePrice
          : nativeValue,
      ...
      snapshot_batch_id: args.snapshotBatchId || null,
      source: args.source,
      bucket_interval: "raw",
    })
    .then(
...
function walkNumbersByKey(
  value: any,
  matcher: (keyPath: string, key: string) => boolean,
  prefix = "",
  depth = 0,
): number[] {
  ...
  if (Array.isArray(value))
    return value.flatMap((item, index) =>
      walkNumbersByKey(item, matcher, `${prefix}.${index}`, depth + 1),
    );
  ...
  for (const [key, raw] of Object.entries(value)) {
    ...
    if (matcher(path, key.toLowerCase())) {
      const amount = asNumber(
        (raw as any)?.amount ?? (raw as any)?.value ?? raw,
        NaN,
      );
      if (Number.isFinite(amount)) found.push(amount);
    }
    if (raw && typeof raw === "object")
      found.push(...walkNumbersByKey(raw, matcher, path, depth + 1));
  }
  return found;
}
  ...
  return (
    candidates.find(
      (n) =>
        Number.isFinite(n) &&
        Math.abs(n) > 0.009 &&
        (!maxValue || Math.abs(n) <= Math.max(maxValue, 1) * 1.05),
    ) || 0
  );
}

function accountCashBreakdownFromPayload(
  account: any,
  holdingsPayload: any,
  balanceValue: number,
  ...
  const dividendCandidates = walkNumbersByKey(
    payload,
    (path) =>
      /(dividend|reinvest|distribution|income|cash_interest)/i.test(path) &&
      /(cash|amount|value|balance|total)/i.test(path) &&
      !/(position|holding|market_value|total_value|invested|equity_value|portfolio_value)/i.test(
        path,
      ),
  );
  const freeCashCandidates = walkNumbersByKey(
    payload,
    (path) =>
      /(available|settled|uninvested|free_cash|cash_available|cash_balance|buying_power|cashbalance)/i.test(
        path,
      ) &&
      !/(margin|loan|requirement|fee|interest_rate|market_value|total_value|position|holding|portfolio_value|equity_value|invested)/i.test(
        path,
      ),
  );
  const totalCashCandidates = walkNumbersByKey(
    payload,
    (path) =>
      /(total_cash|cash_total|cash\.total|cash_amount|cash\.amount|cash\.value|cash_balance)/i.test(
        path,
      ) &&
      !/(market_value|total_value|portfolio_value|equity_value|position|holding|invested)/i.test(
        path,
      ),
  );
  ...
  const inferred =
    balanceValue > 0 && positionsValue > 0 ? balanceValue - positionsValue : 0;

  const partsTotal = Math.max(0, explicitFree) + Math.max(0, dividendCash);
  const total =
    Math.abs(partsTotal) >= 0.01
      ? partsTotal
  ...
  const investable = Math.max(
    0,
    Math.abs(explicitFree) >= 0.01
      ? explicitFree
      : total - Math.max(0, dividendCash || 0),
  );
  return {
    ...
    source:
      Math.abs(partsTotal) >= 0.01
        ...
        : Math.abs(explicitTotal) >= 0.01
          ? "provider_cash_total"
          : Math.abs(inferred) >= 0.5
            ? "balance_minus_positions"
            : "none",
  };
}

// --- POSITION & LOT NORMALIZATION ---
...
function taxLotCurrentValue(position: any) {
  ...
  return lots.reduce(
    (sum: number, lot: any) => sum + asNumber(lot?.current_value, 0),
    0,
  );
}
...
function taxLotCostBasis(position: any) {
  ...
  return lots.reduce(
    (sum: number, lot: any) =>
      sum +
      asNumber(lot?.cost_basis ?? lot?.original_value ?? lot?.book_value, 0),
    0,
  );
}
...
function collectSnapTradePositionCandidates(payload: any, depth = 0): any[] {
  ...
  if (Array.isArray(payload)) {
    return payload.flatMap((item) =>
      collectSnapTradePositionCandidates(item, depth + 1),
    );
  }
  ...
  return keys.flatMap((key) =>
    collectSnapTradePositionCandidates(payload?.[key], depth + 1),
  );
}

function isLondonPencePosition(
  exchange?: string | null,
  currency?: string | null,
) {
  const ex = String(exchange || "")
...
function normaliseSnapTradePriceAndValue(args: {
  exchange?: string | null;
  currency?: string | null;
  units: number;
  latestPrice: number;
  ...
  if (isLondonPencePosition(args.exchange, args.currency)) {
    ...
    const valueLooksLikePence =
      String(args.currency || "").toUpperCase() === "GBX" &&
      value > 0 &&
      rawPenceValue > 0 &&
      Math.abs(value - rawPenceValue) < Math.max(1, value * 0.02);
    const latestLooksLikePence =
      String(args.currency || "").toUpperCase() === "GBX" || latestPrice > 20;
    if (latestLooksLikePence) latestPrice = latestPrice / 100;
    if (
      averageBuyPrice > 20 ||
      String(args.currency || "").toUpperCase() === "GBX"
    )
      averageBuyPrice = averageBuyPrice / 100;
    ...
    .map((position: any, index: number) => {
      ...
      const averageBuyPrice = asNumber(
        position?.average_purchase_price ??
          ...
          position?.book_price,
        Number.isFinite(costBasis) && costBasis > 0 && units
          ? costBasis / units
          : 0,
      );
      ...
      const normalisedCostBasis =
        Number.isFinite(costBasis) && costBasis > 0
          ...
          : 0;
      const hasProviderLots = (Array.isArray(position?.tax_lots) && position.tax_lots.length > 0) || (Array.isArray(position?.lots) && position.lots.length > 0);
      const costBasisSource = normalisedCostBasis > 0
        ? hasProviderLots ? "snaptrade_tax_lots" : averageBuyPrice > 0 ? "snaptrade_average_purchase_price" : "snaptrade_total_cost"
        : null;
      const costBasisVerified = Boolean(normalisedCostBasis > 0 && costBasisSource);
      return {
        ...
        costBasisVerified,
        costBasisSource,
        logoUrl: logoUrlFromPosition(position),
        currency: isLondonPencePosition(symbol.exchange, currency)
          ? "GBP"
          : currency,
        value: normalised.value,
        raw: {
          ...position,
          loop_cost_basis_verified: costBasisVerified,
          loop_cost_basis_source: costBasisSource,
          loop_logo_url: logoUrlFromPosition(position),
        },
      };
    })
    .filter(
...
function providerLotsFromSnapTradePosition(position: SnapTradePositionPreview) {
  ...
  return rawLots
    .map((lot: any, index: number) => {
      ...
      const purchasePrice =
        units > 0 && totalCost > 0 ? totalCost / units : rawPrice;
      ...
      return {
        ...
        notes:
          "Imported from provider tax-lot/cost-basis data. Review if the broker later changes lot detail.",
      };
    })
    .filter((lot: any) => lot.units > 0 && lot.purchasePrice >= 0);
}

async function saveProviderPurchaseLots(
  supabase: SupabaseServer,
  args: {
    appUserId: string;
    holdingId: string;
    position: SnapTradePositionPreview;
  },
) {
  ...
  await supabase
    ...
    .insert(
      lots.map((lot: any) => ({
        ...
        fees: lot.fees,
        price_quote_unit:
          String(args.position.currency || "GBP").toUpperCase() === "GBX"
            ? "gbx"
            : "gbp",
...
type NormalisedSnapTradeActivity = {
  id: string;
  type: string;
  tradeDate: string;
  ticker: string | null;
...
function normaliseSnapTradeActivities(payload: any): NormalisedSnapTradeActivity[] {
  const rows = Array.isArray(payload) ? payload : Array.isArray(payload?.data) ? payload.data : Array.isArray(payload?.activities) ? payload.activities : [];
  return rows.map((row: any, index: number) => {
    const symbol = symbolFromPosition(row);
    const type = firstText(row?.type, row?.activity_type, row?.transaction_type, row?.action, "OTHER").toUpperCase();
    const tradeDate = firstText(row?.trade_date, row?.settlement_date, row?.date, row?.created_at, new Date().toISOString()).slice(0, 10);
    const units = asNumber(row?.units ?? row?.quantity ?? row?.shares, 0);
    const price = asNumber(row?.price ?? row?.unit_price ?? row?.trade_price, 0);
    const amount = asNumber(row?.amount?.amount ?? row?.amount ?? row?.net_amount ?? row?.value, units * price);
    return {
      id: firstText(row?.id, row?.activity_id, row?.transaction_id, `${tradeDate}:${type}:${symbol.ticker || index}:${amount}`),
      type,
      tradeDate,
      ticker: symbol.rawTicker || symbol.ticker || null,
      units,
      price,
      amount,
      currency: currencyFrom(row) || null,
      raw: row,
    };
  }).filter((row: NormalisedSnapTradeActivity) => Boolean(row.id));
}

async function syncSnapTradeAccountActivities(
  supabase: SupabaseServer,
  args: {
    appUserId: string;
    ...
    snapUserId: string;
    userSecret: string;
    holdingIdByTicker: Map<string, string>;
  },
) {
  const endDate = new Date().toISOString().slice(0, 10);
  const start = new Date();
  start.setFullYear(start.getFullYear() - 3);
  const authQuery = `userId=${encodeURIComponent(args.snapUserId)}&userSecret=${encodeURIComponent(args.userSecret)}&startDate=${start.toISOString().slice(0, 10)}&endDate=${endDate}`;
  const payloadRows: any[] = [];

  try {
    for (let offset = 0; offset < 5000; offset += 1000) {
      const payload = await snapTradeRequest<any>("GET", `/accounts/${encodeURIComponent(args.externalAccountId)}/activities?${authQuery}&limit=1000&offset=${offset}`);
      const page = normaliseSnapTradeActivities(payload);
      payloadRows.push(...page);
      if (page.length < 1000) break;
    }
  } catch (err) {
    console.error(`[SnapTrade] Failed to fetch activities:`, err);
    if (!payloadRows.length) return { imported: 0, available: false };
  }

  const activities = Array.from(new Map(payloadRows.map((activity) => [activity.id, activity])).values());
  console.log(`[SnapTrade] Fetched ${activities.length} total historical activities for account.`);

  if (!activities.length) return { imported: 0, available: true };

  // 1. Save Raw Activities with explicit error handling
  const { error: activityError } = await supabase.from("investment_provider_activities").upsert(
    activities.map((activity) => ({
      user_id: args.appUserId,
      investment_account_id: args.localAccountId,
      provider: "snaptrade",
      external_account_id: args.externalAccountId,
      external_activity_id: activity.id,
      activity_type: activity.type,
      activity_date: activity.tradeDate,
      ticker: activity.ticker,
      units: activity.units || null,
      unit_price: activity.price || null,
      amount: activity.amount || null,
      currency: activity.currency,
      raw_payload: activity.raw,
      last_seen_at: new Date().toISOString(),
    })),
    { onConflict: "user_id,provider,external_activity_id" }
  );

  if (activityError) {
    console.error(`[SnapTrade DB Error] Failed to save raw activities:`, activityError.message);
  }

  // 2. Save Buy Lots and match against current holdings
  const purchaseTypes = new Set(["BUY", "REI", "REINVESTMENT", "DIVIDEND_REINVESTMENT", "STOCK_DIVIDEND", "TRADE"]);
  let savedLots = 0;

  for (const activity of activities) {
    if (!purchaseTypes.has(activity.type) || !activity.ticker || activity.units <= 0) continue;
    
    // Robust Matching: Try exact ticker match, then clean suffix (.US, .L, etc.)
    const rawTicker = activity.ticker.toUpperCase();
    const cleanTicker = rawTicker.split('.')[0]; 
    const holdingId = args.holdingIdByTicker.get(rawTicker) || args.holdingIdByTicker.get(cleanTicker);

    if (!holdingId) {
      console.log(`[SnapTrade] Skipped buy lot for ${rawTicker}: Could not match to a current holding.`);
      continue;
    }

    const purchasePrice = activity.price > 0 ? activity.price : activity.amount > 0 ? activity.amount / activity.units : 0;
    
    const { error: lotError } = await supabase.from("investment_purchase_lots").upsert({
      user_id: args.appUserId,
      holding_id: holdingId,
      purchase_date: activity.tradeDate,
      units: activity.units,
      purchase_price: purchasePrice,
      total_cost: activity.amount > 0 ? Math.abs(activity.amount) : activity.units * purchasePrice,
      fees: 0,
      price_quote_unit: String(activity.currency || "GBP").toUpperCase() === "GBX" ? "gbx" : "gbp",
      external_transaction_id: activity.id,
      external_source: "snaptrade_activity",
      notes: activity.type === "REI" || activity.type.includes("REINVEST") ? "Dividend reinvestment imported from SnapTrade account activity." : "Purchase activity imported from SnapTrade account activity.",
    }, { onConflict: "user_id,external_source,external_transaction_id" });

    if (lotError) {
      console.error(`[SnapTrade DB Error] Failed to save purchase lot for ${rawTicker}:`, lotError.message);
    } else {
      savedLots++;
    }
  }

  console.log(`[SnapTrade] Successfully mapped and saved ${savedLots} historical purchase lots.`);
  return { imported: activities.length, available: true };
}

// --- SELF-HEALING COST RECONSTRUCTION ---
// Mathematically reconstructs missing cost basis from historical buy/reinvest lots

async function reconstructMissingCostBasis(
  supabase: SupabaseServer,
  appUserId: string,
  localAccountId: string
) {
  const { data: missingHoldings, error } = await supabase
    .from("investment_holdings")
    .select("id, ticker, units, average_buy_price, imported_invested_value, currency, cost_basis_status")
    .eq("user_id", appUserId)
    .eq("investment_account_id", localAccountId)
    .eq("external_provider", "snaptrade")
    .neq("record_status", "archived")
    .or("average_buy_price.is.null,average_buy_price.lte.0,cost_basis_status.eq.missing");

  if (error) {
    console.error(`[CostReconstruction Error] Failed to query missing holdings:`, error.message);
    return { reconstructed: 0 };
  }

  if (!missingHoldings?.length) {
    console.log(`[CostReconstruction] No holdings found with missing cost prices.`);
    return { reconstructed: 0 };
  }

  console.log(`[CostReconstruction] Found ${missingHoldings.length} holdings missing cost prices. Attempting ledger math...`);

  let reconstructedCount = 0;

  for (const holding of missingHoldings) {
    const holdingUnits = Number(holding.units || 0);
    if (holdingUnits <= 0) continue;

    const { data: lots, error: lotError } = await supabase
      .from("investment_purchase_lots")
      .select("units, purchase_price, total_cost, price_quote_unit")
      .eq("user_id", appUserId)
      .eq("holding_id", holding.id)
      .gt("units", 0)
      .neq("estimated", true) // Ignore synthetic pie projections
      .in("external_source", ["snaptrade", "snaptrade_activity"]); // Trust real SnapTrade lots

    if (lotError) {
      console.error(`[CostReconstruction Error] Failed to fetch lots for holding ${holding.ticker}:`, lotError.message);
      continue;
    }

    if (!lots || lots.length === 0) {
      console.log(`[CostReconstruction] Holding ${holding.ticker} has no historical purchase lots available yet.`);
      continue;
    }
    ...
    for (const lot of lots) {
      ...
      let lotPrice = Number(lot.purchase_price || 0);
      let lotCost = Number(lot.total_cost || (lotUnits * lotPrice));

      // Normalise LSE Pence (GBX) to GBP
      if (
        String(lot.price_quote_unit).toLowerCase() === "gbx" && 
        String(holding.currency).toUpperCase() === "GBP"
      ) {
        lotPrice = lotPrice / 100;
        lotCost = lotCost / 100;
      }

      totalLotUnits += lotUnits;
      totalLotCost += lotCost;
    }

    if (totalLotUnits <= 0 || totalLotCost <= 0) continue;

    const reconstructedAveragePrice = totalLotCost / totalLotUnits;
    const reconstructedInvestedValue = holdingUnits * reconstructedAveragePrice;

    const { error: patchError } = await supabase
      .from("investment_holdings")
      .update({
        average_buy_price: reconstructedAveragePrice,
        imported_invested_value: reconstructedInvestedValue,
        cost_basis_status: "reconstructed_from_activities",
        notes: "Cost basis reconstructed mathematically from historical SnapTrade activity lots because the brokerage omitted live position book cost.",
    ...
    if (patchError) {
      console.error(`[CostReconstruction Error] Failed to patch ${holding.ticker}:`, patchError.message);
    } else {
      console.log(`[CostReconstruction] Successfully mathematically repaired ${holding.ticker} cost basis to ${reconstructedAveragePrice}`);
      reconstructedCount += 1;
    }
  }

  return { reconstructed: reconstructedCount };
}

// --- ACCOUNT & SECRET FETCHING ---
...
async function fetchSnapTradeAccountPositionPayloads(
  ...
  const attempts = [
    {
      ...
      path: `/accounts/${encodeURIComponent(externalAccountId)}/positions/all?${authQuery}`,
    },
    {
      source: "positions",
      path: `/accounts/${encodeURIComponent(externalAccountId)}/positions?${authQuery}`,
    },
    {
      source: "holdings",
      path: `/accounts/${encodeURIComponent(externalAccountId)}/holdings?${authQuery}`,
    },
  ];
  const errors: string[] = [];
  ...
  return {
    payload: null,
    positions: [] as SnapTradePositionPreview[],
    source: "none",
    errors,
  };
}
...
export async function fetchSnapTradeAccountsForUser(
  ...
  const { data: existingAccounts } = await supabase
    ...
    .select(
      "id, label, provider, account_type, external_account_id, external_connection_id, external_account_raw, record_status",
    )
    .eq("user_id", appUserId)
    .eq("external_provider", "snaptrade")
    .neq("record_status", "archived");
  ...
  const previews = await Promise.all(
    activeAccounts.map(async (account: any) => {
      ...
      const cashBreakdown = accountCashBreakdownFromPayload(
        account,
        holdingsPayload,
        balanceValue,
        positionsValue,
      ...
      return {
        ...
        alreadyImported:
          ...
          importedInstitutionKeys.has(
            stableInstitutionAccountKey({ ...previewBase, raw: account }),
          ),
        importGuidance:
...
function labelForAccount(account: SnapTradeAccountPreview) {
  ...
  const wrapper =
    account.wrapperLabel && account.wrapperLabel !== "Unknown wrapper"
      ? account.wrapperLabel
...
export async function importSnapTradeAccountsForUser(
  ...
  for (const account of selected) {
    const accountSnapshotAt = new Date();
    const accountSnapshotBatchId = randomUUID();
    const existing = await supabase
      .from("investment_accounts")
      .select(
        "id, provider_cash_value, provider_investable_cash_value, provider_dividend_cash_value, provider_cash_source",
      )
      .eq("user_id", appUserId)
      .eq("external_provider", "snaptrade")
    ...
    const rawCashSource =
      firstText(account.raw?.loop_cash_source, null) || null;
    const hasProviderCashParts =
      rawCashSource === "provider_cash_parts" ||
      rawCashSource === "provider_cash_total";
    const previousCashSource =
      firstText(existing.data?.provider_cash_source, null) || null;
    ...
    const accountPayload = {
      ...
      provider_dividend_cash_value: preserveManualCash
        ...
        : Math.abs(Number(account.raw?.loop_dividend_cash_value || 0)) >= 0.01
          ...
          : null,
      provider_cash_source: preserveManualCash
        ? previousCashSource
        : rawCashSource,
      provider_isa_subscribed_amount:
        Number(
          account.raw?.loop_isa_subscribed_amount ||
            account.raw?.isa?.subscribed_amount ||
            0,
        ) || null,
      provider_isa_remaining_amount:
        Number(
          account.raw?.loop_isa_remaining_amount ||
            account.raw?.isa?.remaining_amount ||
            0,
        ) || null,
      provider_isa_allowance_year:
        firstText(
          account.raw?.loop_isa_allowance_year,
          account.raw?.isa?.allowance_year,
          null,
        ) || null,
    ...
    const { data: existingActiveProviderHoldings } = await supabase
      ...
      .eq("external_provider", "snaptrade")
      .neq("record_status", "archived");
    const hasExistingRealSnapTradePositions = (
      existingActiveProviderHoldings || []
    ).some(
      (holding: any) =>
    ...
    if (realPositions.length > 0) {
      const currentPositionIds = new Set(
        realPositions
          .map((position) => position.externalPositionId)
          .filter(Boolean),
      );
      ...
      const staleIds = (staleHoldings || [])
        .filter((holding: any) => {
          ...
          return (
            externalPositionId && !currentPositionIds.has(externalPositionId)
          );
        })
    ...
    for (const position of positionsToImport) {
      ...
      const providerResultValue = asNumber(
        position.raw?.profit_loss ??
          position.raw?.p_l ??
          position.raw?.unrealized_pnl ??
          position.raw?.unrealized_profit_loss ??
          position.raw?.day_pnl ??
          position.raw?.daily_pnl,
        NaN,
      );
      const holdingPayload = {
        ...
        imported_invested_value:
          position.costBasis && position.costBasis > 0
            ? position.costBasis
            : null,
        imported_result_value: Number.isFinite(providerResultValue)
          ? providerResultValue
          : null,
      ...
      if (position.ticker && localHoldingId) holdingIdByTicker.set(String(position.ticker).toUpperCase(), localHoldingId);
      await saveProviderPositionSnapshot(supabase, {
        ...
        snapshotAt: accountSnapshotAt,
        snapshotBatchId: accountSnapshotBatchId,
      });
    ...
    await syncSnapTradeAccountActivities(supabase, {
      appUserId,
      localAccountId,
      externalAccountId: account.externalAccountId,
      snapUserId: loopSnapTradeUserId(appUserId),
      userSecret: await getSnapTradeSecretForUser(supabase, appUserId),
      holdingIdByTicker,
    }).then(() => null, () => null);

    // ---> SELF-HEALING COST RECONSTRUCTION TRIGGERED HERE <---
    await reconstructMissingCostBasis(supabase, appUserId, localAccountId).then(() => null, () => null);
...
export async function runSnapTradeProviderSnapshotJob({
  ...
  const { data: profiles, error } = await supabase
    ...
    .or(
      "market_data_provider_status.eq.connected,market_data_realtime_enabled.eq.true",
    )
    .limit(maxUsers);
  if (error) throw new Error(error.message);
  ...
  for (const profile of profiles || []) {
    ...
    try {
      ...
      result.syncedHoldings += synced.imported.reduce(
        (sum, item) => sum + Number(item.holdings || 0),
        0,
      );
    } catch (error) {
